...
class SpecialProductController extends AdminController
{

    function setup(){
        $this->crud->set_url('product-special');
        $this->crud->set_model(SpecialProduct::class);
        $this->crud->set_title(mb_ucfirst('special product'));
    ...
    function create(){
        return $this->admin_view('create.special-product');
    }

    function edit($id){
        $item = $this->crud->get_item($id);
        return $this->admin_view('edit.special-product', [
            'item' => $item,
            'id' => $id,
        ]);
    }
    ...
    function update($id, $request = []){
        ...
        $validator = Validator::make($request->all(), [
            'product_id' => 'required|unique:special_products,product_id,'.$id,
            'type' => 'required|in:Platinum,Gold,Premium',
       ...
       if(($is_permanently == null) || ($is_permanently == 0)){
        ...
        try{
            ...
            $update->is_permanently = $is_permanently;
    ...
    function store(Request $request){
        ...
        $validator = Validator::make($request->all(), [
            'product_id' => 'required|unique:special_products,product_id',
            'date_start' => 'date:yyyy-mm-dd|nullable',
            'type' => 'required|in:Platinum,Gold,Premium',
            'date_end' => 'date:yyyy-mm-dd|nullable',
            'is_permanently' => 'nullable',
       ...
       if(($is_permanently == null) || ($is_permanently == 0)){
        ...
        try{
            ...
            $create->is_permanently = $is_permanently;
            $create->date_start = $date_start;
            $create->date_end = $date_end;
            $create->save();

            DB::commit();
            flash()->addSuccess('Your special product has been submitted.');
            return redirect($this->crud->url);
        }catch (\Exception $e){
                DB::rollBack();
                flash()->addError($e->getMessage());
